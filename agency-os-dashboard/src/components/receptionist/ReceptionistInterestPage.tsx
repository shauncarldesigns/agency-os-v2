...
export function ReceptionistInterestPage({ showToast }: { showToast: ShowToast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [leadsResult, voiceResult] = await Promise.all([api.leads.list({ status: 'not_interested' }), api.voice.overview()]);
