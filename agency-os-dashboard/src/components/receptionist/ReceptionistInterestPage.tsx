...
import { Bot, Building2, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Download, ExternalLink, FlaskConical, KeyRound, LayoutDashboard, Mail, MessageSquare, PhoneCall, PhoneIncoming, Play, RefreshCw, RotateCcw, Send, Settings2, ShieldCheck, Users } from 'lucide-react';
...
type ReceptionistSection = 'overview' | 'leads' | 'setup' | 'demo' | 'calls' | 'qa' | 'integration';
const receptionistNavigation = [
  { key: 'overview', label: 'Overview', icon: LayoutDashboard },
  { key: 'leads', label: 'Interested leads', icon: Users },
  { key: 'setup', label: 'Business setup', icon: Building2 },
  { key: 'demo', label: 'Demo access', icon: KeyRound },
  { key: 'calls', label: 'Calls', icon: PhoneCall },
  { key: 'qa', label: 'Testing & QA', icon: FlaskConical },
  { key: 'integration', label: 'Integration', icon: Settings2 },
] as const satisfies ReadonlyArray<{ key: ReceptionistSection; label: string; icon: typeof LayoutDashboard }>;
...
export function ReceptionistInterestPage({ showToast }: { showToast: ShowToast }) {
  ...
  const [companySwitcherCollapsed, setCompanySwitcherCollapsed] = useState(false);
  ...
  return <div className="main"><div className="mx-auto max-w-6xl space-y-5">
    ...
    <details className="rounded-2xl border border-slate-200 bg-white shadow-sm lg:hidden">
      <summary className="flex cursor-pointer list-none items-center gap-3 p-4"><span className="rounded-lg bg-blue-50 p-2 text-blue-700"><Settings2 className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-[10px] font-semibold uppercase tracking-wide text-slate-400">Command center</span><span className="block truncate text-sm font-semibold text-slate-800">{selectedProfile?.business_name ?? 'Choose a company'}</span></span><ChevronRight className="h-4 w-4 text-slate-400" /></summary>
      <div className="space-y-2 border-t border-slate-100 p-3"><select aria-label="Choose receptionist company" value={selectedProfile?.id ?? ''} onChange={(event) => { const profile = overview?.profiles.find((item) => item.id === Number(event.target.value)); if (profile) openProfile(profile, 'overview'); }} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs text-slate-700"><option value="">Choose a company…</option>{overview?.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.business_name}</option>)}</select><div className="grid grid-cols-2 gap-1">{receptionistNavigation.map((item) => <button type="button" key={item.key} onClick={() => setActiveSection(item.key)} className={`rounded-lg px-3 py-2 text-left text-xs font-semibold ${activeSection === item.key ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-600'}`}>{item.label}</button>)}</div></div>
    </details>

    <div className="flex items-start gap-4">
      <aside className={`sticky top-4 hidden shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:block ${companySwitcherCollapsed ? 'w-16' : 'w-64'}`}>
        <div className="flex items-center gap-2 border-b border-slate-100 p-3">
          {!companySwitcherCollapsed && <div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-wide text-blue-600">Command center</p><p className="truncate text-xs font-semibold text-slate-800">Automated receptionist</p></div>}
          <button type="button" onClick={() => setCompanySwitcherCollapsed((current) => !current)} className="ml-auto rounded-lg p-2 text-slate-500 hover:bg-slate-100" title={companySwitcherCollapsed ? 'Open company switcher' : 'Collapse company switcher'}>{companySwitcherCollapsed ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}</button>
        </div>
        {!companySwitcherCollapsed && <div className="border-b border-slate-100 p-3">
          <label className="block text-[9px] font-semibold uppercase tracking-wider text-slate-400">Selected company<select aria-label="Choose receptionist company" value={selectedProfile?.id ?? ''} onChange={(event) => { const profile = overview?.profiles.find((item) => item.id === Number(event.target.value)); if (profile) openProfile(profile, 'overview'); }} className="mt-1.5 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold normal-case text-slate-700 outline-none focus:border-blue-300"><option value="">Choose a company…</option>{overview?.profiles.map((profile) => <option key={profile.id} value={profile.id}>{profile.business_name}</option>)}</select></label>
        </div>}
        <div className="space-y-1 p-2">
          {receptionistNavigation.map((item) => { const Icon = item.icon; const count = item.key === 'leads' ? interestedLeads.length : item.key === 'calls' ? (overview?.calls ?? []).filter((call) => !selectedProfile || call.voice_business_profile_id === selectedProfile.id).length : null; return <button type="button" key={item.key} title={item.label} onClick={() => setActiveSection(item.key)} className={`flex w-full items-center gap-2.5 rounded-xl p-2.5 text-left text-xs font-semibold ${activeSection === item.key ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}><Icon className="h-4 w-4 shrink-0" />{!companySwitcherCollapsed && <><span className="min-w-0 flex-1 truncate">{item.label}</span>{count !== null && <span className={`rounded-full px-1.5 py-0.5 text-[9px] ${activeSection === item.key ? 'bg-white/20' : 'bg-slate-100'}`}>{count}</span>}</>}</button>; })}
        </div>
      </aside>

      <div className="min-w-0 flex-1 space-y-5">
    <section className={`rounded-2xl border bg-white px-5 py-4 shadow-sm ${selectedProfile?.lead_source === 'local-receptionist-flow-test' && activeSection !== 'leads' ? 'border-violet-400 ring-1 ring-violet-100' : 'border-slate-200'}`}><p className="text-[10px] font-semibold uppercase tracking-wide text-blue-600">{receptionistNavigation.find((item) => item.key === activeSection)?.label}</p><div className="mt-1 flex flex-wrap items-center gap-2"><h3 className="min-w-0 flex-1 truncate text-lg font-semibold text-slate-900">{activeSection === 'leads' ? 'Receptionist sales queue' : selectedProfile?.business_name ?? 'Select a company'}</h3>{selectedProfile?.lead_source === 'local-receptionist-flow-test' && activeSection !== 'leads' && <span className="rounded-full bg-violet-100 px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-violet-700">Test lead</span>}{selectedProfile && activeSection !== 'leads' && <ProfileStateBadge label={`Profile: ${selectedProfile.status}`} tone={selectedProfile.status === 'ready' || selectedProfile.status === 'live' ? 'green' : 'slate'} />}</div></section>

    {activeSection === 'overview' && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-semibold uppercase tracking-wide text-blue-600">Demo workflow</p><h3 className="mt-1 font-semibold text-slate-900">Prepare → Test → Create invitation → Send demo → Review calls</h3><p className="mt-1 text-xs leading-5 text-slate-500">Profiles are managed here. Retell remains the shared voice engine underneath them.</p></div><span className="rounded-full bg-emerald-50 px-3 py-1.5 text-[10px] font-semibold text-emerald-700">Invitation flow ready</span></div>
    </section>}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-2">
          {overview?.profiles.length ? overview.profiles.map((profile) => {
            ...
            const invitation = overview.invitations?.find((item) => item.voice_business_profile_id === profile.id && item.status === 'active');
            const isTestLead = profile.lead_source === 'local-receptionist-flow-test';
            return <div key={profile.id} className={`rounded-xl border p-3 ${isTestLead ? 'border-violet-400 bg-violet-50/30 ring-1 ring-violet-100' : selectedProfile?.id === profile.id ? 'border-blue-300 bg-blue-50/70' : 'border-slate-200'}`}><div className="flex items-start gap-3"><button type="button" onClick={() => openProfile(profile, 'overview')} className="mt-0.5 rounded-lg bg-white p-2 text-blue-600 shadow-sm"><Bot className="h-4 w-4" /></button><button type="button" onClick={() => openProfile(profile, 'overview')} className="min-w-0 flex-1 text-left"><span className="flex items-center gap-2"><span className="truncate text-sm font-semibold text-slate-900">{profile.business_name}</span>{isTestLead && <span className="shrink-0 rounded-full bg-violet-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-violet-700">Test lead</span>}</span><span className="mt-0.5 block text-[11px] text-slate-500">{profile.profile_kind === 'prospect' ? 'Prospect demo' : profile.profile_kind === 'customer' ? 'Customer profile' : 'Internal fallback'} · {calls} call{calls === 1 ? '' : 's'}</span></button><button type="button" onClick={() => openProfile(profile)} className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-semibold text-slate-600">Edit</button></div><div className="mt-3 flex flex-wrap gap-1.5"><ProfileStateBadge label={`Profile: ${profile.status}`} tone={profile.status === 'ready' || profile.status === 'live' ? 'green' : 'slate'} /><ProfileStateBadge label={agentReady ? 'Retell: shared agent ready' : 'Retell: not connected'} tone={agentReady ? 'blue' : 'amber'} /><ProfileStateBadge label={invitation ? `Invitation: ${invitation.use_count ? 'used' : 'sent'}` : 'Invitation: not created'} tone={invitation ? 'green' : 'slate'} /></div><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => openProfile(profile)} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-[10px] font-semibold text-slate-600">Edit setup</button><button type="button" onClick={() => openProfile(profile, 'demo')} className="rounded-lg border border-blue-200 bg-blue-50 px-2 py-2 text-[10px] font-semibold text-blue-700">Send demo</button><button type="button" onClick={() => openProfile(profile, 'calls')} className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-[10px] font-semibold text-slate-600">View calls</button></div></div>;
    ...
    {activeSection === 'leads' && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    ...
    {activeSection === 'integration' && <div className="space-y-5"><SetupChecklist health={overview?.health} expanded /><RetellAuthenticationCard health={overview?.health} /><WebhookRecoveryTools overview={overview} onChanged={load} showToast={showToast} /><RetentionCard overview={overview} onChanged={load} showToast={showToast} /></div>}
      </div>
    </div>
  </div></div>;
}
...
function ProfileDemoPanel({ mode, profile, sourceLead, overview, working, setWorking, onChanged, showToast }: { mode: 'setup' | 'demo'; profile: VoiceBusinessProfile; sourceLead: Lead | null; overview: VoiceOverview | null; working: boolean; setWorking: (value: boolean) => void; onChanged: () => Promise<void>; showToast: ShowToast }) {
  ...
  const activate = async () => {
    if (!callerPhone.trim()) { showToast('Enter your mobile number—the phone you will place the test call from', 'error'); return; }
    setWorking(true);
  ...
  const cancelLiveDemo = async () => {
    setWorking(true);
    try {
      const { canceled } = await api.voice.cancelProfileDemoSessions(profile.id);
      await onChanged();
      showToast(canceled ? 'Merged-demo access ended. Your next call will require a code.' : 'No active merged-demo access remained.', 'success');
    } catch (error) { showToast((error as Error).message, 'error'); } finally { setWorking(false); }
  };
  const useAccessCodeMode = async () => {
    if (!callerPhone.trim()) { showToast('Enter the phone number you will call from', 'error'); return; }
    setWorking(true);
    try {
      const { canceled } = await api.voice.useAccessCodeMode(callerPhone);
      await onChanged();
      showToast(canceled ? 'Conference access cleared. Claire will request your code on the next call.' : 'Access-code mode is ready. Claire will request your code on the next call.', 'success');
    } catch (error) { showToast((error as Error).message, 'error'); } finally { setWorking(false); }
  };
  const sendInvitation = async () => {
    if (!inviteEmail.trim()) { showToast('Enter the prospect email address', 'error'); return; }
    setSendingInvitation(true);
    try { await save(); const { invitation } = await api.voice.sendDemoInvitation(profile.id, inviteEmail, 7); await onChanged(); showToast(`Demo sent to ${invitation.recipient_email} · code ${invitation.access_code}`, 'success'); }
    catch (error) { showToast((error as Error).message, 'error'); } finally { setSendingInvitation(false); }
  };
  ...
  const latestInvitation = overview?.invitations?.find((invitation) => invitation.voice_business_profile_id === profile.id);
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    {mode === 'demo' && <><section className="border-t border-blue-100 bg-blue-50 px-5 py-4"><div className="flex items-start gap-3"><Mail className="mt-0.5 h-4 w-4 text-blue-600" /><div className="min-w-0 flex-1"><h4 className="text-sm font-semibold text-blue-950">Email a demo code for later</h4><p className="mt-1 text-[11px] leading-5 text-blue-700">The six-digit code selects this company. No phone activation is required—the caller simply calls the shared number and gives Claire the code.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><input type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="prospect@business.com" className="h-10 min-w-0 flex-1 rounded-lg border border-blue-200 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-blue-100" /><button type="button" onClick={() => void sendInvitation()} disabled={working || sendingInvitation || !inviteEmail.trim()} className="h-10 rounded-lg bg-blue-600 px-4 text-xs font-semibold text-white disabled:opacity-50">{sendingInvitation ? 'Sending…' : 'Create code & send demo'}</button></div>{latestInvitation && <div className="mt-3 rounded-xl border border-blue-200 bg-white/70 p-3"><p className="text-[11px] text-blue-900">Latest: <strong>{latestInvitation.access_code}</strong> · {latestInvitation.recipient_email} · {latestInvitation.use_count} call{latestInvitation.use_count === 1 ? '' : 's'} · expires {new Date(`${latestInvitation.expires_at.replace(' ', 'T')}Z`).toLocaleDateString()}</p><p className="mt-2 text-[10px] leading-4 text-blue-700">Testing from a phone previously used for a conference demo? Enter that number below to clear its conference authorization first.</p><div className="mt-2 flex flex-col gap-2 sm:flex-row"><input aria-label="Phone that will test the access code" value={callerPhone} onChange={(event) => setCallerPhone(event.target.value)} placeholder="Phone you will call from" className="h-9 min-w-0 flex-1 rounded-lg border border-blue-200 bg-white px-3 text-xs outline-none focus:ring-2 focus:ring-blue-100" /><button type="button" onClick={() => void useAccessCodeMode()} disabled={working || !callerPhone.trim()} className="h-9 rounded-lg border border-blue-300 bg-white px-3 text-[11px] font-semibold text-blue-800 disabled:opacity-50">Use code mode on this phone</button></div></div>}</div></div></section>
    <section className="border-t border-slate-100 px-5 py-4"><div className="grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-semibold text-amber-900">Try it together now · conference call only</p><p className="mt-1 text-[11px] leading-4 text-amber-700">This authorizes the phone Shaun is calling <strong>from</strong> and intentionally bypasses access codes. Use it only when adding Claire to a live merged call.</p><input aria-label="Your conference phone number" value={callerPhone} onChange={(event) => setCallerPhone(event.target.value)} placeholder="Phone placing the conference call" className="mt-2 h-10 w-full rounded-lg border border-amber-200 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-amber-100" /><button type="button" onClick={() => void activate()} disabled={working} className="mt-2 inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-amber-600 px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-50"><Play className="h-3.5 w-3.5" /> Authorize this phone for a merged demo</button>{overview?.activeDemo?.voice_business_profile_id === profile.id && <div className="mt-2 flex flex-wrap items-center justify-between gap-2"><p className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700"><Clock3 className="h-3.5 w-3.5" /> Conference mode until {new Date(`${overview.activeDemo.expires_at.replace(' ', 'T')}Z`).toLocaleTimeString()}</p><button type="button" onClick={() => void cancelLiveDemo()} disabled={working} className="rounded-lg border border-amber-300 bg-white px-2.5 py-1.5 text-[10px] font-semibold text-amber-800 disabled:opacity-50">Stop conference mode · require a code</button></div>}</div>{overview?.health.mode === 'mock' && <div className="rounded-xl border border-slate-200 p-3"><p className="text-xs font-semibold text-slate-800">Local event fixtures</p><p className="mt-1 text-[11px] text-slate-500">Create synthetic records without making a Retell call.</p><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => void mockCall('new_customer')} disabled={working} className="rounded-lg bg-emerald-50 px-2 py-2 text-[11px] font-semibold text-emerald-700">Customer</button><button type="button" onClick={() => void mockCall('cold_sales')} disabled={working} className="rounded-lg bg-amber-50 px-2 py-2 text-[11px] font-semibold text-amber-700">Sales</button><button type="button" onClick={() => void mockTransferFailure()} disabled={working} className="rounded-lg bg-rose-50 px-2 py-2 text-[11px] font-semibold text-rose-700">Failure</button></div></div>}</div></section></>}
  </div>;
}
...
function DesktopTestChecklist() {
  const groups = [
    ...
    ['Behavior', 'Run customer, emergency, sales, uncertain, and prompt-injection scenarios; then inspect the saved eight-case QA details.'],
