...
function ProfileDemoPanel({ mode, profile, sourceLead, overview, working, setWorking, onChanged, showToast }: { mode: 'setup' | 'demo'; profile: VoiceBusinessProfile; sourceLead: Lead | null; overview: VoiceOverview | null; working: boolean; setWorking: (value: boolean) => void; onChanged: () => Promise<void>; showToast: ShowToast }) {
  ...
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    {mode === 'demo' && <><section className="border-t border-blue-100 bg-blue-50 px-5 py-4"><div className="flex items-start gap-3"><Mail className="mt-0.5 h-4 w-4 text-blue-600" /><div className="min-w-0 flex-1"><h4 className="text-sm font-semibold text-blue-950">Send an asynchronous demo</h4><p className="mt-1 text-[11px] leading-5 text-blue-700">Emails the shared number and a unique six-digit code. The frozen profile expires after seven days.</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><input type="email" value={inviteEmail} onChange={(event) => setInviteEmail(event.target.value)} placeholder="prospect@business.com" className="h-10 min-w-0 flex-1 rounded-lg border border-blue-200 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-blue-100" /><button type="button" onClick={() => void sendInvitation()} disabled={working || sendingInvitation || !inviteEmail.trim()} className="h-10 rounded-lg bg-blue-600 px-4 text-xs font-semibold text-white disabled:opacity-50">{sendingInvitation ? 'Sending…' : 'Create code & send demo'}</button></div>{latestInvitation && <p className="mt-2 text-[11px] text-blue-800">Latest: <strong>{latestInvitation.access_code}</strong> · {latestInvitation.recipient_email} · {latestInvitation.use_count} call{latestInvitation.use_count === 1 ? '' : 's'} · expires {new Date(`${latestInvitation.expires_at.replace(' ', 'T')}Z`).toLocaleDateString()}</p>}</div></div></section>
    <section className="border-t border-slate-100 px-5 py-4"><div className="grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-semibold text-amber-900">Try it together now · merged call</p><p className="mt-1 text-[11px] leading-4 text-amber-700">Enter the mobile number you will call <strong>from</strong>. Calls from that number bypass access codes until this access ends.</p><input aria-label="Your test phone number" value={callerPhone} onChange={(event) => setCallerPhone(event.target.value)} placeholder="Your mobile number" className="mt-2 h-10 w-full rounded-lg border border-amber-200 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-amber-100" /><button type="button" onClick={() => void activate()} disabled={working} className="mt-2 inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-amber-600 px-3 py-2.5 text-xs font-semibold text-white disabled:opacity-50"><Play className="h-3.5 w-3.5" /> Activate merged demo for 30 minutes</button>{overview?.activeDemo?.voice_business_profile_id === profile.id && <div className="mt-2 flex flex-wrap items-center justify-between gap-2"><p className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700"><Clock3 className="h-3.5 w-3.5" /> Active until {new Date(`${overview.activeDemo.expires_at.replace(' ', 'T')}Z`).toLocaleTimeString()}</p><button type="button" onClick={() => void cancelLiveDemo()} disabled={working} className="rounded-lg border border-amber-300 bg-white px-2.5 py-1.5 text-[10px] font-semibold text-amber-800 disabled:opacity-50">End merged-demo access</button></div>}</div>{overview?.health.mode === 'mock' && <div className="rounded-xl border border-slate-200 p-3"><p className="text-xs font-semibold text-slate-800">Local event fixtures</p><p className="mt-1 text-[11px] text-slate-500">Create synthetic records without making a Retell call.</p><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => void mockCall('new_customer')} disabled={working} className="rounded-lg bg-emerald-50 px-2 py-2 text-[11px] font-semibold text-emerald-700">Customer</button><button type="button" onClick={() => void mockCall('cold_sales')} disabled={working} className="rounded-lg bg-amber-50 px-2 py-2 text-[11px] font-semibold text-amber-700">Sales</button><button type="button" onClick={() => void mockTransferFailure()} disabled={working} className="rounded-lg bg-rose-50 px-2 py-2 text-[11px] font-semibold text-rose-700">Failure</button></div></div>}</div></section></>}
  </div>;
}
