...
export function SalesIntelligencePage() {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="mt-4 space-y-3">{data.jobs.filter(job => job.status === 'completed' && job.analysis).length === 0 ? <div className="py-4 text-sm text-slate-400">No completed analyses yet.</div> : data.jobs.filter(job => job.status === 'completed' && job.analysis).map(job => <div key={job.id} className="overflow-hidden rounded-xl border border-slate-200"><button onClick={() => setExpandedCallId(current => current === job.call_id ? null : job.call_id)} className="flex w-full items-center justify-between gap-3 bg-slate-50 px-4 py-3 text-left"><div><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-slate-900">{job.company}</span><OutcomeAgreement status={job.outcome_reconciliation.status} /></div><div className="mt-0.5 text-xs text-slate-500">Call #{job.call_id} · operator: {job.outcome} · transcript: {String(job.analysis?.outcome || 'unknown').replaceAll('_', ' ')}</div></div><span className="text-xs font-semibold text-blue-700">{expandedCallId === job.call_id ? 'Hide report' : 'View report'}</span></button>{job.recording_url && <div className="border-t border-slate-200 bg-slate-50 px-4 py-3"><AuthenticatedAudioPlayer url={job.recording_url} compact lazy /></div>}{expandedCallId === job.call_id && <CallAnalysisDetail analysis={job.analysis!} report={reports[job.call_id]} hasRecording={Boolean(job.recording_url)} removing={removingCallId === job.call_id} onRemove={async (deleteRecording) => { const warning = deleteRecording ? `Permanently remove the ${job.company} call from Sales Intelligence AND delete its original recording?\n\nThe audio cannot be recovered. The call-history entry will remain.` : `Remove the ${job.company} call from Sales Intelligence?\n\nIts transcript, analysis, and reporting data will be deleted. The original recording and call history will be preserved.`; if (!window.confirm(warning)) return; setRemovingCallId(job.call_id); setNotice(''); try { await api.calls.removeIntelligence(job.call_id, deleteRecording); setExpandedCallId(null); setReports(current => { const next = { ...current }; delete next[job.call_id]; return next; }); setNotice(`${job.company} was removed from Sales Intelligence.${deleteRecording ? ' Its recording was permanently deleted.' : ' Its recording is still saved.'}`); await load(); } catch (e) { setNotice(e instanceof Error ? e.message : 'Could not remove this call from Sales Intelligence.'); } finally { setRemovingCallId(null); } }} />}</div>)}</div>
    </section>
...
function CallAnalysisDetail({ analysis, report, hasRecording, removing, onRemove }: { analysis: Record<string, unknown>; report?: CallIntelligenceReport; hasRecording: boolean; removing: boolean; onRemove: (deleteRecording: boolean) => Promise<void> }) {
  ...
  return <div className="space-y-4 px-4 py-4 text-sm text-slate-700">
    ...
    <div className="border-t border-slate-200 pt-4"><div className="flex flex-wrap gap-2"><button type="button" disabled={removing} onClick={() => void onRemove(false)} className="rounded-lg border border-rose-200 bg-white px-3 py-2 text-xs font-semibold text-rose-700 hover:bg-rose-50 disabled:cursor-wait disabled:opacity-60">{removing ? 'Removing…' : 'Remove analysis only'}</button>{hasRecording && <button type="button" disabled={removing} onClick={() => void onRemove(true)} className="rounded-lg bg-rose-600 px-3 py-2 text-xs font-semibold text-white hover:bg-rose-700 disabled:cursor-wait disabled:opacity-60">Delete analysis + recording</button>}</div><p className="mt-1.5 text-xs text-slate-500">Both options remove the call from Sales Intelligence. The call-history entry remains.</p></div>
  </div>;
}
