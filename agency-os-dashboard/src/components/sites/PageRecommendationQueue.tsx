...
import { ArrowRight, CheckCircle2, FilePlus2, LoaderCircle, RefreshCw } from 'lucide-react';
...
export function PageRecommendationQueue({ project, hasMaster, showToast, onOpenBrief, onRecommendationKeysChange, onOptimizationPageIdsChange, onOpenConfiguration, onPageChanged }: {
  ...
  return <section className="mb-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="space-y-3 p-5">
      ...
      {optimizationRecommendations.length > 0 && <div className="rounded-xl border border-orange-200"><div className="border-b border-orange-100 bg-orange-50 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-orange-700">Page updates requiring action</div><div className="divide-y divide-slate-100">{optimizationRecommendations.map((item) => <div key={item.id} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-slate-800">{item.title}</p>{item.description && <p className="mt-1 text-xs leading-relaxed text-slate-500">{item.description}</p>}</div>{item.page_id && <button type="button" disabled={!!busyKey} onClick={() => void openOrGenerateOptimizationBrief(item)} className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg bg-orange-600 px-3 py-2 text-xs font-semibold text-white hover:bg-orange-700 disabled:opacity-50">{busyKey === `work:${item.id}` ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <FilePlus2 className="h-4 w-4" />}{item.brief_id ? 'Open update brief' : 'Generate update brief'}</button>}</div>)}</div></div>}
