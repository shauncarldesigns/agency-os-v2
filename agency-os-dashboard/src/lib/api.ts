...
export const api = {
  ...
  messaging: {
    ...
    conversation: async (id:number) => {const payload=await apiFetch<any>(`/api/messaging/conversations/${id}`);payload.messages=payload.messages.map((message:any)=>{if(!message.attachments_json)return message;try{const attachments=JSON.parse(message.attachments_json).map((attachment:any)=>({...attachment,preview_url:attachment.preview_url?.startsWith('/')?`${API_BASE}${attachment.preview_url}`:attachment.preview_url}));return {...message,attachments_json:JSON.stringify(attachments)};}catch{return message;}});return payload;},
    send: (id:number, body:string, attachmentIds:number[] = []) => apiFetch<any>(`/api/messaging/conversations/${id}/send`, { method:'POST', body:JSON.stringify({body,attachmentIds}) }),
    uploadAttachment: async (conversationId:number,file:File) => {const form=new FormData();form.append('file',file,file.name);const res=await fetch(`${API_BASE}/api/messaging/conversations/${conversationId}/attachments`,{method:'POST',credentials:'include',headers:authHeaders(),body:form});if(!res.ok){const payload=await res.json().catch(()=>({}));throw new ApiError(payload.error||`Upload failed: ${res.status}`,res.status);}const payload=await res.json();payload.attachment.preview_url=`${API_BASE}${payload.attachment.preview_url}`;return payload;},
    deleteAttachment: (id:number) => apiFetch<any>(`/api/messaging/attachments/${id}`,{method:'DELETE'}),
