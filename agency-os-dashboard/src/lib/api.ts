...
export const api = {
  ...
  calls: {
    ...
    correctIntelligenceTranscript: (id: number, utterances: string[]) => apiFetch<{ job_id:number; status:string }>(`/api/call-intelligence/calls/${id}/transcript`, { method: 'PATCH', body: JSON.stringify({ utterances }) }),
    swapIntelligenceSpeakers: (id: number) => apiFetch<{ job_id:number; status:string; shaun_speaker:number }>(`/api/call-intelligence/calls/${id}/swap-speakers`, { method: 'POST' }),
