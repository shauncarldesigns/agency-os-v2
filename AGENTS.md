# AGENTS.md — Agency OS v2

Durable project facts for any Codex session. Point-in-time session notes
...
into projects, drive landingsite.ai with Codex-generated briefs. Monorepo:
...
(`Codex.ts`, `places.ts`, `outscraper.ts`, `pagespeed.ts`, `reviewMiner.ts`,
...
- **Generate-rebuttal** — `POST /api/playbook/generate-rebuttal` (Codex
...
## Clarity engagement scoring (added 2026-07-27)

Text Outreach uses two engagement layers:

- **Layer 1:** the app-owned `/r/:lead_id` redirect in `routes/redirect.ts`.
  This is the reliable trigger: increments `pipeline_sessions`, logs
  `lead_activity.action='click_tracked'`, promotes first `sent_no_reply`
  click to `engaged`, and sets a minimum `engagement_score=5`.
- **Layer 2:** Microsoft Clarity export sync in `services/clarity.ts`.
  This is context/scoring, not the primary trigger. It pulls the last 3 days
  from `https://www.clarity.ms/export-data/api/v1/project-live-insights`
  with `dimension1=URL`, matches aggregate records to leads via
  `clarity_tag`, `campaign_slug`, or tagged site URLs, and updates
  `engagement_score`, `engagement_grade`, `engagement_reasons`,
  `clarity_last_sync_at`, and `pipeline_sessions`.

**Config/secrets:**
- `CLARITY_PROJECT_ID` is a non-secret Worker var. Current shared demo-site
  project id: `xt0tg8n14n`.
- `CLARITY_API_TOKEN` is a Worker secret, created from Clarity project
  Settings → Data Export → Generate new API token.

**Install model:** One shared Clarity project for all outreach demo sites,
not one project per lead. Demo-site briefs append a `CLARITY TRACKING INSTALL
BLOCK` with the shared script plus lead-specific tags:
`clarity("set", "lead", "lead-{id}")`, campaign slug, and company name. The
operator should paste this into landingsite's global header/custom-code area.

**Sync entrypoints:**
- Hourly cron piggybacks on the existing `0 * * * *` scheduled handler.
- Manual endpoint: `POST /api/pipeline/clarity-sync`.
- Snippet endpoint: `GET /api/pipeline/leads/:id/clarity-snippet`.

**Engagement buckets:** `hot` ≥ 90 ("Call immediately"), `walkthrough` ≥ 70,
`follow_up` ≥ 40, otherwise `nurture`. `opportunity_score` remains the
pre-outreach prospect-quality score; `engagement_score` is post-demo behavior.
...
## Messaging Employee (added 2026-08-10)

Communications is the Twilio-backed SMS inbox and outbound employee. It reuses
...
those sources with newly invented language.

Public Twilio endpoints are `/webhooks/twilio/sms/inbound` and
`/webhooks/twilio/sms/status`; both validate `X-Twilio-Signature` and remain
outside `/api/*` authentication. Dashboard APIs live at `/api/messaging/*`.
TEST + mock transport is the default. Production is locked until the sender,
Messaging Service, test phone, and external A2P registration are configured.

Secrets/config: `TWILIO_MESSAGING_SERVICE_SID`, `TWILIO_PHONE_NUMBER`,
...
from Text Outreach; when the lead has a valid email and built demo, the existing
Email Outreach automation is scheduled.

## Builder Employee (added 2026-08-10)

`builder-worker/` is an explicitly started bulk Playwright employee that turns
a snapshot of eligible `awaiting_build` leads into `ready_to_send`, strictly one
site at a time. D1 tables `builder_runs`, `builder_jobs`, `builder_events`, and
`builder_control` own run history, queue/audit, the live activity timeline,
safe-point stop/pause, and live step state.
The Worker exposes a dedicated bearer-token API at `/api/builder-worker/*` and
operator controls at `/api/builder/*`; secret: `BUILDER_API_TOKEN`.

LandingSite has two creation fields: Business Name receives `leads.company` and
Business Description receives `leads.pipeline_brief`. Completion is state-based:
click `Preview Website` as soon as it appears, capture the `*.agcy.dev` URL, and
keep checking that same preview until it no longer shows `Preview not found` or
`Getting your preview ready`. The existing pipeline URL-save
service remains authoritative for tagging and the `ready_to_send` transition.

Email Outreach is build-first: a lead with an email and no existing website is
shown in its Awaiting Build lane without requiring an `Email Captured` call
outcome. Start Builder generates missing `pipeline_brief` values in the
application before taking the queue snapshot. When the Builder returns a URL,
`completePipelineBuild()` schedules email automation if the lead has a valid
email. The browser employee never generates or edits prompts.

The Builder dashboard separates employee health, run state, and active-job
state. It also exposes readiness/missing-brief counts, current progress, job
results and diagnostics, `builder_events`, median/24-hour metrics, and the
latest twelve runs. Idle `/claim` polls refresh worker presence so an online
but idle employee is not reported as offline.

Migration `2026-08-10-builder-employee.sql` must be applied manually before the
backend containing the queue insert is deployed.

## Models & APIs

- **Brief generation model:** `BRIEF_MODEL` in `routes/briefs.ts` = `Codex-opus-4-7`.
  **Opus 4.7 rejects the `temperature` param** — `services/Codex.ts` strips it for
  models matching `/opus-4-7/`. Brief variance comes from the angle-led prompt, not
  temperature. Review mining uses Haiku (`Codex-haiku-4-5-...`), which DOES honor it.
...
- **Keep the docs in sync on shipping PRs.** As part of the same branch (or a
  ...
  - `AGENTS.md` — update when the PR changes durable facts (new env secrets,
