...
-- Messaging Employee — Twilio-backed Communications Center.
...
CREATE TABLE IF NOT EXISTS messaging_conversations (
  id INTEGER PRIMARY KEY AUTOINCREMENT, lead_id INTEGER REFERENCES leads(id),
  phone_number TEXT NOT NULL, channel TEXT NOT NULL DEFAULT 'sms',
  ai_mode TEXT NOT NULL DEFAULT 'auto', status TEXT NOT NULL DEFAULT 'open',
  needs_human INTEGER NOT NULL DEFAULT 0, needs_human_reason TEXT,
  unread_count INTEGER NOT NULL DEFAULT 0, is_test INTEGER NOT NULL DEFAULT 0,
  last_message_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE UNIQUE INDEX IF NOT EXISTS idx_messaging_conversation_phone_test ON messaging_conversations(phone_number,is_test);
...
CREATE TABLE IF NOT EXISTS messaging_ai_audit (
  ...
  confidence REAL NOT NULL, script_key TEXT, generated_response TEXT, auto_sent INTEGER NOT NULL DEFAULT 0,
  escalated INTEGER NOT NULL DEFAULT 0, provider TEXT NOT NULL DEFAULT 'rules',
  model TEXT NOT NULL DEFAULT 'controlled-v1', created_at TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE TABLE IF NOT EXISTS messaging_scripts (
  script_key TEXT PRIMARY KEY, label TEXT NOT NULL, body TEXT, approved INTEGER NOT NULL DEFAULT 0,
  source_reference TEXT, updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
('price_question','Price question',NULL,0),('who_is_this','Who is this',NULL,0),
('what_is_this','What is this',NULL,0),('already_has_website','Already has website',NULL,0),
('positive_interest','Positive interest',NULL,0),('wants_call','Wants call',NULL,0),
('not_interested','Not interested',NULL,0),('follow_up_later','Follow up later',NULL,0),
('did_not_receive_link','Missing link',NULL,0),('human_escalation','Human escalation acknowledgment',NULL,0);
