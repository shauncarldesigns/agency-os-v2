...
CREATE TABLE IF NOT EXISTS call_intelligence_jobs (
  ...
  status TEXT NOT NULL DEFAULT 'queued' CHECK (status IN ('queued','transcribing','analyzing','completed','failed')),
  attempt_count INTEGER NOT NULL DEFAULT 0, locked_at TEXT, error TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')), started_at TEXT, completed_at TEXT,
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  UNIQUE(call_id, requested_prompt_version)
);
CREATE INDEX IF NOT EXISTS idx_call_intelligence_jobs_status ON call_intelligence_jobs(status, created_at);
...
CREATE TABLE IF NOT EXISTS call_analyses (
  ...
  analysis_prompt_version TEXT NOT NULL, analysis_schema_version TEXT NOT NULL,
  analysis_json TEXT NOT NULL, call_type TEXT NOT NULL, outcome TEXT NOT NULL,
  outcome_confidence REAL, overall_score INTEGER, recommended_next_action TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')), superseded_at TEXT,
  UNIQUE(call_id, analysis_prompt_version, analysis_schema_version)
);
CREATE INDEX IF NOT EXISTS idx_call_analyses_current ON call_analyses(call_id, superseded_at, created_at DESC);
CREATE TABLE IF NOT EXISTS call_analysis_facts (
  ...
  category TEXT NOT NULL, reaction TEXT, confidence REAL, quote TEXT, timestamp TEXT,
  PRIMARY KEY (analysis_id, fact_type, category, timestamp)
);
CREATE INDEX IF NOT EXISTS idx_call_analysis_facts_reporting ON call_analysis_facts(fact_type, category, reaction);
...
-- Automated Receptionist — Retell-backed profiles, personalized demos, calls,
-- and idempotent provider event intake. Customer callers remain separate from
-- Agency OS sales leads.
