...
VALUES
  ...
  ('Email Preview - Warm Sales Call', '(920) 555-0115', 'call-board-seed'),
...
-- Email Outreach engaged example. This deliberately uses the call route and
-- Email Captured outcome so it appears in Email Outreach rather than Text Outreach.
UPDATE leads SET
  contact = 'Taylor',
  email = 'taylor@example.com',
  industry = 'plumber',
...
SELECT id, 'email_captured', 'ready_to_send', 'sent_no_reply',
       ...
       datetime('now', '-2 days')
FROM leads WHERE company = 'Email Preview - Warm Sales Call';
